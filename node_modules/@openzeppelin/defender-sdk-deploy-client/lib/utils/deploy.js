...
function extractArtifact({ artifactPayload: artifact, contractName: name, contractPath: path, }) {
    ...
    if (!source)
        throw new Error(`Contract ${name} source not found in artifact.input ${artifact}`);
    if (!settings)
        throw new Error(`Contract ${name} settings not found in artifact.input ${artifact}`);
    if (!language)
        throw new Error(`Contract ${name} language not found in artifact.input ${artifact}`);
    return {
        input: {
