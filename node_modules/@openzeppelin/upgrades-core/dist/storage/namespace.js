...
const integer_literals_1 = require("../utils/integer-literals");
...
class CustomLayoutClashWithNamespaceError extends error_1.UpgradesError {
    constructor(id, contractName, src) {
        super(`Custom layout for contract ${contractName} clashes with the storage location for namespace ${id}`, () => `\
 ...
 */
function checkCustomLayoutClashWithNamespace(contractDef, storageLocation, contractName, src) {
    if (contractDef.storageLayout !== undefined && storageLocation.startsWith(erc7201_1.ERC7201_FORMULA_PREFIX)) {
        const namespaceId = storageLocation.substring(erc7201_1.ERC7201_FORMULA_PREFIX.length);
        const calculatedStorageLocation = (0, erc7201_1.calculateERC7201StorageLocation)(namespaceId);
        if (contractDef.storageLayout.baseSlotExpression.nodeType === 'Literal') {
            const baseSlotNormalized = (0, integer_literals_1.normalizeUint256Literal)(contractDef.storageLayout.baseSlotExpression.value); // TODO: when Solidity supports an erc7201 helper function, also check that for clash
...
function warnIfCustomLayoutAndNamespacesFound(namespacesWithSrc, decodeSrc, origContractDef) {
    ...
    if (Object.entries(namespacesWithSrc).length > 0 && origContractDef.storageLayout !== undefined) {
        ...
        (0, log_1.logWarning)(`${decodeSrc(origContractDef.storageLayout)}: Ensure custom storage layout does not overlap with namespaced storage layout.`, [
